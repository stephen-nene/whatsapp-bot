...
async function processUserMessage(accountSid, body) {
  ...
  if (!currentSession) {
    ...
    return handleNewSession(accountSid, body, twimlResponse);
  }
...
function handleNewSession(accountSid, twimlResponse) {
  const response = new twiml.MessagingResponse();
  // console.log(MessageTemplates.WELCOME())
  response.message(
    `👋 Welcome! Please send your registration number to proceed.`
  );
  SessionManager.initializeSession(userSessions, accountSid);
